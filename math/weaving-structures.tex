...
\[
  \mathcal{C}
  =
  \{b\in\{0,1\}^{S_0}: b\le r \text{ coordinatewise for some }
  r\in\mathcal{R}\}.
\]
...
\[
  \mathcal{R}\subseteq\mathcal{U}_R(\mathcal{B}),
\]
then choose, for every row type \(r\in\mathcal{R}\), a representation
\[
  r=b_{t_1}\vee\cdots\vee b_{t_k},
  \qquad
  k\le R,
  \qquad
  b_{t_\ell}\in\mathcal{B}.
\]
...
\[
  ...
  =
  \begin{cases}
    (b_t)_s, & 1\le s\le S_0,\ 1\le t\le m,\\
    0, & \text{otherwise}.
  \end{cases}
\]
For each pick \(i\), use the chosen representation of \(r(i)\) and set
\[
  A_{i t}
  =
  \begin{cases}
    1, & b_t \text{ is used in the chosen representation of } r(i),\\
    0, & \text{otherwise}.
  \end{cases}
\]
...
appears in a union producing a row \(r\), then it must be coordinatewise
contained in \(r\). Therefore every useful treadle vector lies in the candidate
