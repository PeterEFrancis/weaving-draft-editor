...
\[
  \Phi=\Phi_\beta.
\]
It may use the same co-clustering search to produce candidate reduced triples
\((\tau,\chi,U)\), and the same conversion from reduced triples to admissible
draft matrices \(A,B,C\). The difference is ranking and acceptance: candidate
drafts are evaluated by the movement objective
\[
  \Phi_\beta(\widehat{D},D^\ast),
  \qquad
  \widehat{D}_{ij}=\mathbf{1}\{(AB^TC)_{ij}>0\}.
\]
Hamming error remains a useful diagnostic, but it is no longer the primary
objective. In particular, a draft that shifts a motif slightly may beat a draft
with fewer isolated cell disagreements if the latter requires more total black
cell movement to repair.

Computing \(\Phi_\beta\) is itself an optimization problem. It can be handled by
a minimum-cost flow formulation, or by an assignment formulation with ordinary
movement costs \(d_1(u,v)\) together with deletion and insertion choices with
costs \(b(u)\) and \(b(v)\). Since the objective is evaluated on completed
candidates rather than every individual cell update, Beta 1 can reuse much of
the Alpha search machinery while changing the final notion of quality.

\subsection{Beta 2}

Beta 2 uses the optimal movement plan as search pressure. After a candidate
drawdown \(X\) is scored, the minimizing matching \(M\) identifies three kinds
...
For example, if many matched moves point in the same local direction, a repair
step may try shifting the responsible row cluster, column cluster, or treadle
combination before changing the density of the region. If many cells are
scheduled to enter through the edge, the search may prefer modifications that
add black cells near those boundary-to-target paths. If many cells are
scheduled to exit, the search may prefer removing activations that are close to
an edge and weakly supported by the surrounding motif.
...
The practical hypothesis is that \(\Phi_\beta\) rewards visual locality while
the structural pressures discourage drafts that achieve low movement only by
